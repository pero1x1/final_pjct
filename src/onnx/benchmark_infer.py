...
def main():
    ...
    ap.add_argument("--provider", default="CPUExecutionProvider")  # or CUDAExecutionProvider
    args = ap.parse_args()
    ...
    torch_line = (
        f"Torch (cpu)   ms/batch: {fmt_ms(t_torch):.3f} | "
        f"batches/s: {1 / t_torch:.2f} | rows/s: {rps(t_torch):.0f}"
    )
    fp32_line = (
        f"ONNX FP32     ms/batch: {fmt_ms(t_fp32):.3f} | "
        f"batches/s: {1 / t_fp32:.2f} | rows/s: {rps(t_fp32):.0f}"
    )
    int8_line = (
        f"ONNX INT8     ms/batch: {fmt_ms(t_int8):.3f} | "
        f"batches/s: {1 / t_int8:.2f} | rows/s: {rps(t_int8):.0f}"
    )
